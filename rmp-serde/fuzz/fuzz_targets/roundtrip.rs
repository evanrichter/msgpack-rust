...
macro_rules! round_trip {
    ($ty:ty, $data:ident, $equality:expr) => {{
        ...
        let x: Result<$ty, _> = ::rmp_serde::from_slice($data);
        if let Ok(inner) = x {
            #[cfg(feature = "debug")]
            dbg!(&inner);

            let ser = ::rmp_serde::to_vec(&inner).expect("a deserialized type should serialize");
            #[cfg(feature = "debug")]
            dbg!(&ser);

            let des: $ty =
                ::rmp_serde::from_slice(&ser).expect("a serialized type should deserialize");
            #[cfg(feature = "debug")]
            dbg!(&des);
